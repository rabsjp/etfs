...
df$active_a<-(df$player.nbida+df$player.naska)
df$active_b<-(df$player.nbidb+df$player.naskb)
df$active_c<-(df$player.nbidc+df$player.naskc)
...
drisk<-drisk[,c("participant.id_in_session","player.boxes_collected","session.code")]
df<-merge(df,drisk,by=c("participant.id_in_session","session.code"))
...
df$uses<-100000*df$participant.id_in_session+df$session*10
